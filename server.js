...
async function setupDB() {
  try {
    ...
    await pool.query("ALTER TABLE photos ADD COLUMN IF NOT EXISTS fit VARCHAR(10) DEFAULT 'cover'");
...
app.get('/api/photos', async (req, res) => {
  res.set('Cache-Control', 'no-store');
  try {
    var r = await pool.query("SELECT slot, EXTRACT(EPOCH FROM updated_at)::bigint ts, pos_x, pos_y, zoom, fit FROM photos");
    var map = {}; r.rows.forEach(function (x) {
      map[x.slot] = { ts: x.ts, x: x.pos_x == null ? 50 : x.pos_x, y: x.pos_y == null ? 50 : x.pos_y, zoom: x.zoom == null ? 100 : x.zoom, fit: x.fit === 'contain' ? 'contain' : 'cover' };
    });
    res.json(map);
  } catch (e) { res.json({}); }
});
...
app.post('/api/admin/photo-frame', authMiddleware, async (req, res) => {
  try {
    ...
    var fit = req.body.fit === 'contain' ? 'contain' : 'cover';
    if (!slot) return res.status(400).json({ ok: false, error: 'slot fehlt' });
    var r = await pool.query("UPDATE photos SET pos_x=$2, pos_y=$3, zoom=$4, fit=$5 WHERE slot=$1", [slot, x, y, zoom, fit]);
    if (r.rowCount === 0) return res.status(404).json({ ok: false, error: 'Kein eigenes Foto für diesen Bereich' });
    res.json({ ok: true, x: x, y: y, zoom: zoom, fit: fit });
  } catch (e) { res.status(500).json({ ok: false, error: e.message }); }
});
